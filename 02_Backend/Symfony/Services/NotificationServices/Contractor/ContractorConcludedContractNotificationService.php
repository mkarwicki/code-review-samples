...
class ContractorConcludedContractNotificationService {
	private $notificationType = 'contractor-concluded-contract';
	...
	public function sendNotificationEmail(Notifications $notification){
		$this->notificationsEmail->setName('
			Contract '.$notification->getContract()->getContractID().' 
			('.$notification->getJobs()->getTitle().') signed by employer. Contract has now been concluded.
		');

		$this->notificationsEmail->setBody(
			$this->twig->render( 'email_templates/notifications_email/contractor/contractor_concluded_contract.html.twig',
				[
